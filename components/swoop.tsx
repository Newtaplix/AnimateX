...
const Swoop = ({id}:swoopProp) => {
    const current = id
    const words = ["Animate.", "Interact.", "Elevate."]
  return (
    <div>
        <div className='flex mb-5'>
            {
                words.map((item, i) => 
                    <motion.div key={item} className='text-2xl md:text-4xl relative w-fit px-2 md:px-5 text-center items-center flex'>
                        <span>{item}</span>
                    <AnimatePresence>
                        {
                                current === i ?
                                ...
                                className='bg-transparent absolute w-full top-0 left-0 right-0 h-full'>
                                    <div className='w-full h-full relative transparent border-2 border-gray-400 bg-gray-400/8'>
